...
!(async () => {
    // 勿扰模式
    $.isMute = [true, 'true'].includes(
        $.getdata('@chavy_boxjs_userCfgs.isMute')
    );
...
async function apiRevertGlobalBak() {
    ...
    if (backup) {
        ...
        $.setdata(
            JSON.stringify(chavy_boxjs_app_subCaches),
            $.KEY_app_subCaches
        );
        const isNull = (val) =>
...
function Env(name, opts) {
    ...
    return new (class {
        ...
        loaddata() {
            if (this.isNode()) {
                ...
                const isRootDirDataFile =
                    !isCurDirDataFile &&
                    this.fs.existsSync(rootDirDataFilePath);
            ...
            if (/^@/.test(key)) {
                ...
                if (objval) {
                    try {
                        ...
                        val = objedval
                            ? this.lodash_get(objedval, paths, '')
                            : val;
                    } catch (e) {
                        val = '';
                    }
        ...
        get(request, callback = () => {}) {
            ...
            if (
                ...
                if (this.isSurge() || this.isLoon())
                    request['auto-redirect'] = false; // Surge & Loon
                if (this.isQuanX())
                    request.opts
            ...
            switch (this.getEnv()) {
                ...
                case 'Quantumult X':
                    ...
                    $task.fetch(request).then(
                        ...
                        (err) =>
                            callback((err && err.error) || 'UndefinedError')
                    );
                    break;
                case 'Node.js':
                    ...
                    this.got[method](url, _request).then(
                        ...
                        (err) => {
                            ...
                            callback(
                                ...
                                resp,
                                resp &&
                                    iconv.decode(resp.rawBody, this.encoding)
                            );
                        }
                    );
                    break;
                        ...
                        switch (this.getEnv()) {
                            ...
                            case 'Loon': {
                                ...
                                let openUrl =
                                    ...
                                    $open;
                                if (openUrl)
                                    Object.assign(options, { openUrl });

                                let mediaUrl =
                                    rawopts.mediaUrl || rawopts['media-url'];
                                if ($media?.startsWith('http'))
                                    mediaUrl = $media;
                                if (mediaUrl)
                                    Object.assign(options, { mediaUrl });
                            ...
                            case 'Quantumult X': {
                                ...
                                let mediaUrl =
                                    rawopts['media-url'] || rawopts.mediaUrl;
                                if ($media?.startsWith('http'))
                                    mediaUrl = $media;
                                if (mediaUrl)
                                    Object.assign(options, {
        ...
        log(...logs) {
            ...
            console.log(
                logs.map((l) => l ?? String(l)).join(this.logSeparator)
            );
        }
